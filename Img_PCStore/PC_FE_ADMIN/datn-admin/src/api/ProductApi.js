...
export const getAllProductsByBrandAdmin = (brand, page, size) => {
  const url = `/api/site/product/by-brand-admin?brand=${brand}&page=${page}&size=${size}`;
  return Instance.get(url);
};
...
export const updateStatusProduct = (id) => {
  ...
  const url = `/api/admin/product/update-status?id=` + id;
